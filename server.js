...
const models = ["anthropic/claude-opus-4-6","anthropic/claude-sonnet-4-6","google/gemini-3.2-pro","openai/gpt-5.2"];
...
http.createServer((req, res) => {
  ...
  if (req.method === 'GET' && req.url.startsWith('/status')) {
    ...
    res.writeHead(200, { 'Content-Type': 'application/json' });
    res.end(JSON.stringify({ status: user && users[user] ? 'connected' : 'disconnected', model: users[user]?.model || 'anthropic/claude-sonnet-4-6' }));
    return;
  }
