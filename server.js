...
let db = { users: {}, projects: [], tokens: {}, sessions: {} };
  ...
  if (req.method === 'POST' && pathname === '/account') {
    ...
    req.on('end', () => {
      try {
        ...
        const now = Date.now();
        db.users[username] = { hashed, token, created: now };
        db.sessions[username] = { model: 'anthropic/claude-sonnet-4-6', connected: false };
        db.tokens[token] = { username, expires: now + 30*24*60*60*1000 };
        saveDb();
        res.writeHead(200);
  ...
  if (req.method === 'POST' && pathname === '/login') {
    ...
    req.on('end', () => {
      try {
        ...
        if (db.users[username].hashed !== hash(password)) {
          res.writeHead(401);
        ...
        const now = Date.now();
        db.tokens[token] = { username, expires: now + 30*24*60*60*1000 };
        db.users[username].lastLogin = now;
        saveDb();
        res.writeHead(200);
        res.end(JSON.stringify({ success: true, token }));
      } catch (e) {
